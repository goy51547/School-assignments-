...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    for livingGhost in livingGhostPositionDistributions:
        pos=livingGhost.argMax()
        ghostDist+=[self.distancer.getDistance(pacmanPosition, pos)]
        ghostPos+=[pos]
    ghostIndex = ghostDist.index(min(ghostDist))
    distances=[]
    for action in legal:
        successorPosition = Actions.getSuccessor(pacmanPosition, action)
        distances+=[self.distancer.getDistance(successorPosition, ghostPos[ghostIndex])]
    return legal[distances.index(min(distances))]
